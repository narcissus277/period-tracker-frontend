const _URL='https://mxfvgspptteadirmpjqy.supabase.co';
const _KEY='eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im14ZnZnc3BwdHRlYWRpcm1wanF5Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODIxOTQyMzAsImV4cCI6MjA5Nzc3MDIzMH0.HVeDbsECu7v0PVuggG4gnroXZj4aRv7IhR_AJ79BHh0';
async function qry(path,opts){
  var h=new Headers();
  h.set('apikey',_KEY);
  h.set('Authorization','Bearer '+_KEY);
  h.set('Content-Type','application/json');
  h.set('Prefer','return=representation');
  var r=await fetch(_URL+'/rest/v1'+path,
    {method:'GET',headers:h,...opts});
...
async function saveRecord(){
  ...
  try{
    if(selectedOpt==='clear'){
      await qry('/period_records?start_date=eq.'+selectedDate,
        {method:'DELETE'});
      records=records.filter(r=>r.start_date!==selectedDate);
      await qry('/ovulation_records?ovulation_date=eq.'+selectedDate,
        {method:'DELETE'});
      ...
      if(ex){showToast('这天已经记录过排卵期了');}
        ...
        const result=await qry('/ovulation_records',
          {method:'POST',
           body:JSON.stringify(
             {ovulation_date:selectedDate,notes:''})});
        ovulRecords.push(result[0]);
        showToast('已记录排卵期');
      }
      ...
      if(ex){
        await qry('/period_records?start_date=eq.'+selectedDate,
          {method:'PATCH',
           body:JSON.stringify(
             {notes:'marked',duration:1})});
        ex.notes='marked';ex.duration=1;
      }
      else{
        const result=await qry('/period_records',
          {method:'POST',
           body:JSON.stringify(
...
async function deleteRecord(dateStr){
  ...
  try{
    await qry('/period_records?start_date=eq.'+dateStr,
      {method:'DELETE'});
...
async function deleteOvulRecord(dateStr){
  ...
  try{
    await qry('/ovulation_records?ovulation_date=eq.'+dateStr,
      {method:'DELETE'});
...
async function init(){
  try{
    const data=await qry('/period_records?order=start_date.asc');
    records=(data||[]).map(r=>({...r,duration:r.duration||5}));
    const ovulData=await qry('/ovulation_records?order=ovulation_date.asc');
    ovulRecords=(ovulData||[]);
    renderStatus();renderCalendar();
    setTimeout(()=>switchTab(0,false),50);
  }catch(e){
    document.getElementById('status-section').innerHTML=
      `<div class="loading-state" style="color:#e85d75">加载失败：${e.message}</div>`;
  }
}
init();
